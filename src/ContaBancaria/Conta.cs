...
public class Conta
{
    ...
    {
        if (valor <= 0) throw new ArgumentException("O valor do depósito deve ser maior que zero.", nameof(valor));
        if (!Ativa) throw new InvalidOperationException("Não é possível depositar em conta inativa.");
        Saldo += valor;
    }
    ...
    {
        if (valor <= 0) throw new ArgumentException("O valor do saque deve ser maior que zero.", nameof(valor));
        if (!Ativa) throw new InvalidOperationException("Não é possível sacar de conta inativa.");
    ...
    public void Transferir(Conta destino, decimal valor)
    {
        ...
        if (valor <= 0) throw new ArgumentException("O valor da transferência deve ser maior que zero.", nameof(valor));
        if (!Ativa) throw new InvalidOperationException("Conta origem inativa.");
        if (!destino.Ativa) throw new InvalidOperationException("Conta destino inativa.");
        if (valor > Saldo) throw new InvalidOperationException("Saldo insuficiente.");
        Saldo -= valor;
        destino.Saldo += valor;
    }
